...
- **This skill never downloads or fetches a PDF.** A Step 4 high-potential candidate is staged as a `% CANDIDATE <date>` BibTeX entry in `paper/literature/my_base.bib` (same convention lit-review-loop's Phase 1 uses) and the actual download+verify is handed to **`lit-review-loop`'s Phase 2** — shared, owner or me, never silently skipped. Don't write a fetch script; don't hand-roll a `curl`/`pdftotext` pipeline — that's lit-review-loop's job.
- **Check what's already staged/known before searching** (Step 0) — this repo's bib is already ~180 entries; a scoop-check that re-searches from scratch wastes a round-trip and risks re-staging a duplicate.
...
- Existing bib titles/keys: skim `paper/literature/my_base.bib` (grep titles, not abstract bodies — entries carry full abstracts).
- Existing write-up coverage: `grep -nE '^#{1,3} ' text_docs/shared/literature_review.md` (organized by attack/defense/eval taxonomy — §3 image attacks, §5 encoding attacks, §6 defenses, §7 eval).
- Already-downloaded-but-unstaged papers: `ls paper/literature/` — some papers are on disk with no bib entry yet.
...
**Stage, don't fetch.** For each candidate, append a `% CANDIDATE <date>` comment + a BibTeX entry carrying `note = {CANDIDATE — verify+download}` to `paper/literature/my_base.bib` (key style `authorYYYYshorttitle`, matching the file's existing convention) — auto-generated metadata is unverified until downloaded. Then hand off to **`lit-review-loop`'s Phase 2** for the download+verify: offer to do it myself for plain-arXiv candidates (as lit-review-loop already permits), and flag the rest (auth-walled / non-arXiv / camera-ready-judgment) for the owner. Don't proceed to Step 5 for a candidate until its download+verify has actually landed.

## Step 5 — Full-Paper Deep Dive on Candidates

Once lit-review-loop's Phase 2 has landed a candidate's PDF + `source/` in `paper/literature/<paper-name>/`, read it — prefer the `.tex`/HTML `source/` over the PDF (PDF extraction garbles tables/math/figures, per lit-review-loop). Don't hand a PDF URL to `WebFetch` for this — it returns a model-written summary that drops the methodological detail this step exists to capture.
...
- `.claude/skills/lit-review-loop/SKILL.md` — owns Phase 2 (download+verify) and Phase 3 (write-up into `text_docs/shared/literature_review.md`); this skill only stages candidates and hands off.
- `paper/literature/my_base.bib` — candidate + verified bib entries.
- `text_docs/shared/literature_review.md` — existing coverage, checked in Step 0.
