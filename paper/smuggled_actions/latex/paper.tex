...
\title{Smuggled Actions: Encoded Indirect Prompt Injection Evades Surface-Form Defenses on LLM Agents}
\author{
...
\begin{abstract}
% WORKING ABSTRACT (workflow stage S5, pre-results): states the paper's thesis and study
% design. Fill in the headline numbers (per-defense action-success lift; benign-utility bound)
% at write-up, once the payload x defense x scaffold x backbone matrix has been run.
LLM agents act on the data they ingest---tool outputs, retrieved documents, on-screen
images---which turns that data channel into an untrusted surface for \emph{indirect} prompt
injection. The injection-specific defenses now deployed against this threat---spotlighting,
data isolation, and prompt-shield / classifier guards---are tuned to recognize an imperative
natural-language surface in ingested content. We ask whether they inherit the \emph{decode}
blind spot already documented for content-level safety guards: an injected instruction that is
\emph{encoded}---as set theory, formal logic, source code, a cipher, classical Chinese,
homoglyphs, or a rendered image---presents a surface the defense was never tuned on, yet a
capable backbone still decodes it and executes the injected action. We introduce a systematic,
controlled evaluation of encoding as an evasion axis against agent injection defenses, crossing
payload encodings with defenses across multiple agent scaffolds and backbones on standard,
action-scored harnesses (AgentDojo, InjecAgent), with plain-versus-encoded controls that
isolate the encoding effect and a benign-task utility bound that limits over-blocking. Our
analysis separates two defense families: encoding evades \emph{surface-form} defenses that key
on the lexical form of ingested text, whereas a \emph{behavioral} defense that keys on
action-dependence rather than surface form (MELON) resists it---localizing the blind spot to
the surface-form class rather than to agent injection defense in general.
\end{abstract}
