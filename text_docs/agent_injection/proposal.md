...
*Codename: **Smuggled Actions** (Paper E — the alias **E was reassigned from the parked `judge_reliability` direction** in the sibling repo `llm_guardrail_security`, 2026-07-19). Origin: this repo's `text_docs/shared/future_work.md §1` (the agent-side line, migrated from the sibling's future_work §5) + the compositional-harm identity. **Attack-first** — the attack is this paper; the defense is the deliberately-later half (`future_work.md §2`). Full paper-facing title refined at writing.*
