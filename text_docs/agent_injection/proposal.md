...
Checked two candidate new elements layered on encoded injection. Full log `outputs/scoop_check/2026-07-22/`.
- **A — decode-then-act staging (agent decodes downstream of the defense, then acts): Level 2 / High Overlap — NOT a new attack.** This is the CURRENT paper's core mechanism: the decode-capability-asymmetry insight is already owned by Controlled-Release Prompting (`fairoze2026...`, chat domain) and our existing delta already IS "relocate it to the agent data channel + action metric" (§1, lit review §6). Re-pitching it as a new attack would collide with our own frame.
- **A′ — tool-assisted self-decoding (agent invokes its OWN tools — code interpreter / translator — to decode the smuggled action): Level 4–5 / OPEN.** ~30 queries + ~30 abstract deep-checks found no paper doing this. Genuinely new, agent-native, and it DISSOLVES the MathEnc capability tension (impossible in a single-shot model/VLM setting, so no self-contradiction). **Cost:** needs a decode-capable tool in the environment — AgentDojo's stock suites lack one → either add a benign utility tool to a suite (keeps built infra) or use a coding-agent harness (native code tool; cf. `anon2026sokcodinginjection`, 2601.17548).
- **B — single-agent distributed encoded injection (fragments across tool outputs, reassembled in-context): Level 3 / Medium but FRAGILE.** Slice is unoccupied but one axis from `hu2026localmonitorsmisscompositional` (multi-agent encoded) and STAC (single-agent NL); Context Stitching (`anon2026contextstitching`, 2607.14493) proves the mechanism in a non-agent log setting. Re-opens single-agent compositional harm, which §7 CLOSED (Level 1). Keep as an ablation, not the headline.
- **New prior art staged** (`my_base.bib`, verify+download via lit-review-loop Phase 2): `chauhan2026caughtactivation` (2606.04141, activation-level exfil defense using an encoding suite — sharpens our text-level delta), `anon2026contextstitching` (2607.14493), `anon2026sokcodinginjection` (2601.17548). Contrast attacks to cite (evade defenses but NOT via encoding — strengthen "encoding is the axis"): IterInject (2605.24659), AutoDojo (2606.15057), Agent Data Injection (2607.05120).
- **Recommendation:** if elevating to a "new attack," pursue **A′ tool-assisted self-decoding** (the only genuinely-new, agent-native, MathEnc-safe candidate); B stays a secondary ablation. Otherwise the current Level-3 encoded-injection-vs-surface-form-defenses paper stands as-is. **Owner decision pending** (§9 open decisions).
