...
repo `portfolio.md`); IDs are earned at commitment, and dead papers carry no series ID (a rare
post-submission death leaves its number as a tombstone, never reused). Sibling to [`llm_guardrail_security`](https://github.com/vacantfury/llm_guardrail_security),
...
**Origin — the `E` alias.** This paper carries no series ID: it died 2026-08-02, the day before the
paper-ID standard, and dead papers are unnumbered — its handle is the codename plus ex-E. The letter E
was reassigned 2026-08-02 to the model-internals first paper (**AS-5**, repo `model_internals_safety`).
Paper E was spun out to this repo on **2026-07-19** from the sibling
