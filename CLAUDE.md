...
- **The SIBLING `imaging_text_attacks_for_llm_jailbreaking` owns the model-side (VLM) line:** encoding + imaging jailbreak *attacks* and *content-guard* defenses, judged by harm / refusal on the model's TEXT output. Papers A–D live there; `judge_reliability` is parked there.
...
- **LLM provider layer = the `llm_utils` base package** (pinned git dep by tag, e.g. `llm_utils @ git+https://github.com/vacantfury/llm_utils@v2.3.0`; the vendored `src/llm_utils/` copy was removed 2026-07-23). Import `from llm_utils import LLMServiceFactory, LLMModel, ...`; no config loader is wired in this repo, so `LLMServiceFactory.create()` uses caller kwargs only. Upgrades = bump the tag deliberately; never vendor a copy back.
